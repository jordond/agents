...
2. **Unnamed, single-use agents.** A named teammate delivers its result *and* an idle notification, and each costs you a full turn at your current context. Name an agent only for a mid-flight correction. Once an agent reports, it is finished: stop it and spawn a fresh one for follow-up work (review fixes, a retry) with the prior result pasted into the new brief. A reused agent pays its whole old transcript on every new turn. Size every slice to finish under 150k context; 250k is the ceiling and means the slice was cut wrong.
