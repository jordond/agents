...
### 3. Clarify with steering questions
Ask the few questions that actually change the plan (scope boundaries, approach forks, unknowns you couldn't resolve by research). For each, offer **1-3 recommended options** with a brief why, so the user can steer fast instead of composing prose. Skip questions you already answered through research. Prefer the `AskUserQuestion` tool.
