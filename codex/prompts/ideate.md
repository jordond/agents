...
Turn something fuzzy into a plan precise enough that `/workon` can execute it with no further discovery. Plan quality comes from **evidence**, not assumption. Use this — not `/workon` — whenever there is no written plan or issue yet, even if I say "build" or "implement": planning and research come first.

Argument (`$ARGUMENTS`): a TODO, issue, or new idea. Blank → pick from the board.

## Output discipline

You'll do a lot of research, but I only see: the clarifying questions, the continuation prompt, and the final handoff. Don't narrate research as you go. Keep questions tight. Tokens spent reading the codebase are worth it; tokens spent describing your reading are not.
...
- Otherwise read the Status issue and offer the open TODOs (plus "something new"). Let me pick.
...
If you cannot find evidence for something the plan depends on, that's a question for me — not a guess to paper over.
...
- **One question at a time.** Each answer reshapes what to ask next; batching forecloses that. Ask, then wait for my answer (cluster only when questions are genuinely independent).
- **Recommend an answer to every question** — your best pick plus a one-line why — so I confirm or redirect instead of composing prose.
...
Once intent is clear, **stop and let me clear context** (planning research bloats the window; writing the plan fresh is cleaner and cheaper). Give me a paste-ready continuation prompt that carries everything forward, e.g.:
...
When I paste it back, resume at step 5 without re-researching.
...
### 8. Hand off
Stop. Tell me — one line — to clear context and run `/workon <issue#>` (the master issue, if split). Don't start implementing.
